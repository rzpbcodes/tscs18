...
*Instances (1)*:
```solidity
File: BlockhashDependency.sol

36:         uint256 winnerIndex = uint256(blockhash(block.number)) %
...
34:     function chooseWinner() external onlyOwner {
...
39:         payable(contenders[winnerIndex]).transfer(

42:         payable(owner).transfer(1 ether);
...
31:         contendersCount = contendersCount + 1;
...
35:         require(contendersCount > 10, "Not enough contenders registered");
...
27:             msg.value >= 1 ether && msg.value < 2 ether,
...
25:     function register(address contender) external payable {
...
6:     mapping(uint256 => address) private contenders;
...
5:     uint256 private contendersCount;
